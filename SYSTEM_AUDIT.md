...
### أ. ما يمكن حذفه (بدون تأثير على الدقة):
1. **حذف `/api/candidates` Endpoint**: إزالة الكود الميت من `server.js`.
2. **حذف `imageResultCache`**: إزالة كاش الصور المعتمد على Base64 Hash لتوفير الذاكرة، والاعتماد فقط على `resultCache` (Text-based Cache).
3. **حذف طلبات `/api/lookup` من مسار OCR**: الاكتفاء بالبحث المحلي (`searchInDB`) في الفرونت إند. الجداول المحلية تحتوي على نفس البيانات، وهذا سيوفر Network Latency.

### ب. ما يمكن دمجه (لتقليل Latency):
1. **دمج خطوات Gemini (Single-Pass RAG)**:
   - بدلاً من طلب Gemini للقراءة ثم طلب آخر للتصنيف، يمكن إرسال الصورة + قواعد التصنيف الأساسية في **طلب واحد فقط**.
   - إذا كان الكود معروفاً، سيلتقطه الـ OCR المحلي أو الـ Text Cache. إذا وصل الأمر لـ Gemini، فهذا يعني أن الكود معقد ويحتاج لتصنيف مباشر.
   - هذا سيوفر 2-4 ثواني من مسار Gemini.

### ج. ما يمكن تبسيطه:
1. **توحيد دوال التحقق**: دمج `isValidMemoryCode` و `looksLikeMemoryCode` في دالة واحدة قوية.
2. **إيقاف Tesseract فعلياً**: استخدام `Tesseract.terminate()` عند فوز Gemini لإيقاف الـ Web Worker وتحرير موارد الموبايل، بدلاً من الاعتماد فقط على `AbortController` الذي لا يوقف الـ Worker.

---

**الخلاصة**: النظام الحالي (v20) يمتلك معمارية ممتازة (Parallel + Arbitration)، ولكن يمكن جعله أسرع بـ 30-40% عن طريق إزالة طلبات الـ HTTP الزائدة في مسار الـ OCR، ودمج خطوات Gemini في طلب واحد، وتنظيف الكود الميت.
